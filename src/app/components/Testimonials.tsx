...
export function Testimonials() {
  ...
  useEffect(() => {
    ...
    import("../services/catalog")
      .then(({ getPublicReviews }) => getPublicReviews())
      .then((remoteReviews) => {
        if (mounted) setReviews(remoteReviews.slice(0, 6));
      })
      .catch(() => {
    ...
    <section id="testimonios" className="section-shell section-shell--ink">
      <div className="section-inner">
        ...
        <div className="trust-row" aria-label="Atributos de servicio">
          {["Ingredientes naturales", "Empaque premium", "Cotizacion personalizada", "Coordinacion por WhatsApp"].map(
            (item, idx) => (
              <span
                key={item}
